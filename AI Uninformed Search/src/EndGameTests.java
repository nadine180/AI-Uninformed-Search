...
public class EndGameTests {
	...
	private boolean applySnapped(String tPosition, int[] result) {
		return (result[0] + "," + result[1]).equals(tPosition)
				&& result[2] == 6;
	}
	...
	private void applyLeft(char[][] g, int[] result) {
		if (result[1] - 1 >= 0
				&& (g[result[0]][result[1] - 1] == '\u0000'
						|| g[result[0]][result[1] - 1] == 'S' || (g[result[0]][result[1] - 1] == 'T' && result[2] == 6)))
			result[1] = result[1] - 1;
	}
	...
	private void applyDown(char[][] g, int[] result) {
		int m = g.length;
		if (result[0] + 1 < m
				&& (g[result[0] + 1][result[1]] == '\u0000'
						|| g[result[0] + 1][result[1]] == 'S' || (g[result[0] + 1][result[1]] == 'T' && result[2] == 6)))
			result[0] = result[0] + 1;
	...
	private char[][] convertToGrid(String input) {
		...
		char[][] grid = new char[Integer.parseInt(dimensions[0])][Integer
		...
		for (int i = 0; i < warriors.length - 1; i += 2)
			grid[Integer.parseInt(warriors[i])][Integer
					.parseInt(warriors[i + 1])] = 'W';
